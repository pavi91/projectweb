    ...
    public class ReservationController {
         ...
         * @return ControllerResult with list of all reservations
         */
        public ControllerResult<List<ReservationDTO>> listAllReservations() {
            try {
                List<ReservationDTO> reservations = bookingService.getAllReservations();
                logger.info("Retrieved {} reservations", reservations.size());
                return new ControllerResult<>(true, "Reservations retrieved", reservations);
            } catch (Exception e) {
                logger.error("Error listing reservations", e);
                return new ControllerResult<>(false, "Error listing reservations: " + e.getMessage(), null);
            }
        }

        /**
         * List reservations for a specific guest (by guestId)
         * @param guestId the guest's ID (from guests table)
         * @return ControllerResult with list of reservations for this guest
         */
        public ControllerResult<List<ReservationDTO>> listReservationsForGuest(int guestId) {
            try {
                List<ReservationDTO> guestReservations = bookingService.getReservationsByGuest(guestId);
                logger.info("Retrieved {} reservations for guest {}", guestReservations.size(), guestId);
                return new ControllerResult<>(true, "Reservations retrieved", guestReservations);
            } catch (Exception e) {
                logger.error("Error listing reservations for guest {}", guestId, e);
                return new ControllerResult<>(false, "Error listing reservations: " + e.getMessage(), null);
            }
        }

        /**
         * List reservations for a specific guest user
         * NOTE: This previously assumed userId==guestId; now it delegates to listReservationsForGuest
         * after the caller resolves the correct guestId.
         */
        @Deprecated
        public ControllerResult<List<ReservationDTO>> listReservationsForUser(int userId) {
            // This method is kept for backward compatibility but no longer used.
            // It simply returns all reservations filtered in-memory by guestId==userId.
            try {
                List<ReservationDTO> allReservations = bookingService.getAllReservations();
                List<ReservationDTO> guestReservations = allReservations.stream()
                        .filter(res -> res.getGuestId() == userId)
                        .collect(Collectors.toList());

                logger.info("Retrieved {} reservations for user {} (deprecated path)", guestReservations.size(), userId);
                return new ControllerResult<>(true, "Reservations retrieved", guestReservations);
            } catch (Exception e) {
                logger.error("Error listing reservations for user {}", userId, e);
                return new ControllerResult<>(false, "Error listing reservations: " + e.getMessage(), null);
            }
        }
